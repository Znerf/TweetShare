...
function com_content_sharing_init(){
	...
	ossn_extend_view('js/opensource.socialnetwork', 'js/ContentSharing');
	ossn_extend_view('js/opensource.socialnetwork', 'js/vendors/html2canvas.js');
...
function com_content_sharing_add_post_share_link($hook, $type, $return, $params) {
	...
	if($show_entry) {
		...
		ossn_register_menu_item("postextra", array(
				'name' => 'sharepostlink',
				'text' => '<i class="btn btn-primary"><span>'.ossn_print('com_content_sharing:share:postlink').'</span></i>',#ossn_print('com_content_sharing:share:postlink'),
				'href' => $share_url,
				'priority' => 300
		));
	}
